...
# Enhanced function to check if a query is medical-related with stronger classification
...
def chat():
    ...
    try:
        ...
        if not is_medical:
            ...
            # Save conversation to DB if user is logged in, even for rejected queries
            user_id = session.get('user_id')
            if user_id:
                db = get_db_connection()
                if db:
            ...
            return jsonify({'response': non_medical_response})
        ...
        treatments, and other health-related topics. If the user asks anything non-medical,
        you must politely refuse to answer.
        
        Respond with accurate, evidence-based medical information and always include appropriate
        disclaimers about consulting healthcare professionals for diagnosis and treatment.
        """
